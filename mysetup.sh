...
# # script configurationC:\julialva\live-video-analytics-master\live-video-analytics-iot-edge-csharp-master\src\edge
# BASE_URL='C:/julialva/live-video-analytics-master/live-video-analytics-iot-edge-csharp-master/src/edge' # location of remote files used by the script
# DEFAULT_REGION='westus2'
# ENV_FILE='setup/edge-deployment/.env'
# APP_SETTINGS_FILE='setup/appsettings.json'
# #ARM_TEMPLATE_URL="$BASE_URL/deploy.json"
# DEPLOYMENT_MANIFEST_URL="$BASE_URL/deployment.yolov3.template.json"
# DEPLOYMENT_MANIFEST_FILE='setup/edge-deployment/deployment.yolov3.amd64.json'
# ROLE_DEFINITION_URL="$BASE_URL/setup/LVAEdgeUserRoleDefinition.json"
# ROLE_DEFINITION_FILE='setup/role_definition.json'
# RESOURCE_GROUP='teamlvarg'
# APPDATA_FOLDER_ON_DEVICE="/var/local/mediaservices" #you must change this later but it is ok for now
# CLOUD_SHELL_FOLDER="$HOME/clouddrive/live-video-analytics-master/live-video-analytics-iot-edge-csharp-master/src/edge"
...
# #create container registry
# CONTAINER_REGISTRY="lvacontainerregistry"
# echo -e "
# ${YELLOW}What is the name of the container registry to use?${NC}
# This will create a new one if one doesn't exist.
# Hit enter to use the default (${BLUE}${CONTAINER_REGISTRY}${NC})."
...
# echo -e "
# ${YELLOW}What is the name of the storage account to use?${NC}
# This will create a new one if one doesn't exist. When running this script after setting up your Azure Stack Edge device and creating shares, you'll want to ensure that you use the same storage account
# Hit enter to use the default (${BLUE}${STORAGE_ACCOUNT_NAME}${NC})."
# read -p ">> " tmp
# STORAGE_ACCOUNT_NAME=${tmp:-$STORAGE_ACCOUNT_NAME}
...
# else
#     echo "storage account named ${STORAGE_ACCOUNT_NAME} already exists"
# fi
